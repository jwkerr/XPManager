...
public class ExperienceUtil {
    ...
    public static double getLevelFromXP(long xp) {
        ...
        // Get remaining XP progress towards next level, cast to float for next bit of math
    ...
    private static int getXPToNext(int level) {
        if (level >= 30) // Simplified formula. Internal: 112 + (level - 30) * 9
            return level * 9 - 158;

        if (level >= 15) // Simplified formula. Internal: 37 + (level - 15) * 5
            return level * 5 - 38;
